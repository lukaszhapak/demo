...
public class CodingRulesTest {
  ...
  void generalCodingRules() {
	GeneralCodingRules.NO_CLASSES_SHOULD_ACCESS_STANDARD_STREAMS.because("You should use a logger")
		.check(classesToCheck);
	GeneralCodingRules.NO_CLASSES_SHOULD_THROW_GENERIC_EXCEPTIONS.because(
		"You should name your exceptions").check(classesToCheck);
	GeneralCodingRules.NO_CLASSES_SHOULD_USE_JODATIME.because("You should not use joda time")
		.check(classesToCheck);
	GeneralCodingRules.NO_CLASSES_SHOULD_USE_FIELD_INJECTION.because(
		"You should use constructor/setter injection").check(classesToCheck);
  }
}
